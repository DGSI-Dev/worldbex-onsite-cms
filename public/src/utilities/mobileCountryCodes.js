...
var countries = [...new Set(countriesWithNumber)]
.sort((a,b)=>{
...
let mobileList = [] 
let options = countries.map(obj=> {
  mobileList.push(`<option value="${obj.dial_code}" ${obj.code == "PH" ? 'selected' : ''}>${obj.flag} ${obj.dial_code}</option>`)
  if(obj.code != "PH"){
    return `<option value="${obj.code}">${obj.name}</option>`
  }
})

$(".countryMobile").empty().append(mobileList.join(''))
$('[name="country"]').append(
