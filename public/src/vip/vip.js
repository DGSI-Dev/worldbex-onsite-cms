...
let formatStatusChar = (phrase, status) => {
  switch (status) {
    ...
    case '1st LETTER CAPITAL': {
      console.log(phrase, phrase.toLowerCase().replace(/\b\w/g, c => c.toUpperCase()))
      ...
      if (errArr.includes(false)){
        ...
        data.forEach((obj, i) => {
          ...
          if (firstName.trim() || lastName.trim() || companyName.trim()) {
            // let { title, firstName, lastName, companyName } = obj

            obj.attendDate = currentEvent.dateSeries
            obj.title =  formatStatusChar(title, $("#set_title").val())
            obj.firstName = firstName.trim() ? formatStatusChar(firstName.trim(), $("#set_firstName").val()) : 'VIP'
            obj.lastName = lastName.trim() ? formatStatusChar(lastName.trim(), $("#set_lastName").val()) : '-' 
            obj.companyName = formatStatusChar(companyName.trim(), $("#set_companyName").val()) 
            ...
            if (successVIPInLocal) {
                ...
                let isExist =  Boolean(successVIPInsert.find(({ firstName: sFN, lastName: sLN, companyName: sCN, title: sTitle }) =>  
                  sFN == firstName.trim() && sLN == lastName.trim() && sCN == companyName.trim() && sTitle == title
                ))
              if (!isExist) {
                validData.push(obj)
              }
            ...
            console.log('MAY ERROR ROW',i,obj)
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            function getData()
            {
              ...
              $.ajax({
                type:'POST',
                url: '/api/createEventVipTransaction',
                data: {
